...
export async function up(knex){
  await knex.schema.createTable('secciones', (table) => {
    table.bigInteger('id_seccion').primary().notNullable().comment('ID único de la sección');
    table.string('valor_seccion', 1).notNullable().comment('Valor identificador de la sección (ej: "1")');
