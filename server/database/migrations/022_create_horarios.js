...
export async function up(knex){
  await knex.schema.createTable('horarios', (table) => {
    ...
    table.bigInteger('unidad_curricular_id').notNullable().comment('Unidad curricular programada');
    ...
    table.time('hora_inicio').notNullable().comment('Hora de inicio de la clase');
    table.time('hora_fin').notNullable().comment('Hora de finalización de la clase');
    table.boolean('activo').notNullable().defaultTo(true).comment('Indica si el horario está actualmente activo');
    ...
    // Índices únicos compuestos (modificado para evitar el error)
    table.unique(['aula_id', 'dia_semana', 'hora_inicio', 'hora_fin'], 'uq_aula_horario');
    table.unique(['profesor_id', 'dia_semana', 'hora_inicio', 'hora_fin'], 'uq_profesor_horario');
    ...
    // Relaciones (asegurando que las tablas referenciadas existan)
